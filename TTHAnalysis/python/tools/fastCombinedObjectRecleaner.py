...
class fastCombinedObjectRecleaner:
    ...
    def __init__(self,label,inlabel,cleanTausWithLooseLeptons,cleanJetsWithFOTaus,
        ...
        if self.doFwdJets:
            self._helper_fwdjets = CollectionSkimmer("JetFwdSel"+self.label, "JetFwd", floats=self.vars+self.vars_jets, ints=self.vars_jets_int, maxSize=20)
            self._helpers.append(self._helper_fwdjets)
        ...
        if self.doFwdJets:
            self._fwdjet_worker = ROOT.fastCombinedObjectRecleanerHelper(self._helper_taus.cppImpl(),self._helper_fwdjets.cppImpl(),self.cleanJetsWithFOTaus,btagL_thr,btagM_thr)
            for x in self.jetPts: self._fwdjet_worker.addJetPt(x)
    ...
    def initReaders(self,tree):
        collections = ["LepGood","TauGood","Jet"]
        if self.doFwdJets: collections.append("JetFwd")
        for coll in collections:
            ...
            if coll=='JetFwd': _vars.extend(self.vars_jets+self.vars_jets_int+self.vars_jets_nooutput)
            for B in _vars:
    ...
    def initWorkers(self):
        ...
        if self.doFwdJets:
            self._fwdjet_worker.setLeptons(self.nLepGood, self.LepGood_pt, self.LepGood_eta, self.LepGood_phi)
            self._fwdjet_worker.setTaus(self.nTauGood, self.TauGood_pt, self.TauGood_eta, self.TauGood_phi)
            self._fwdjet_worker.setJets(self.nJetFwd,self.JetFwd_pt,self.JetFwd_eta,self.JetFwd_phi,self.JetFwd_btagCSV,self.JetFwd_corr,self.JetFwd_corr_JECUp,self.JetFwd_corr_JECDown)    
    ...
    def __call__(self,event):
        ...
        if self.doFwdJets:
            self._fwdjet_worker.clear()
            self._fwdjet_worker.loadTags(tags,self.cleanTausWithLooseLeptons)
            self._fwdjet_worker.run()
